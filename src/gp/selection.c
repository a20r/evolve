        ...
        while (selected != max_select) {
                ...
                for (i = 0; i < max_pop; i++) {
                        ...
                        if (cumulative_prob >= randnum) {
                                /* keep a reference to individuals and scores */
                                chromo = darray_get(p->individuals, i);
...
void gp_tournament_selection(struct population *p, int *select)
{
        ...
        for (j = 0; j < max_select; j++) {
                ...
                for (i = 0; i < t_size; i++) {
                        ...
                        /* individual and score */
                        chromo = darray_get(p->individuals, r);
                        score = darray_get(p->scores, r);
                ...
                chromo = gp_tree_copy(darray_get(t_chromos, 0));
        ...
        /* clean up */
        for (i = 0; i < t_chromos->end; i++) {
                gp_tree_destroy(darray_get(t_chromos, i));
        }
        darray_destroy(t_chromos);
